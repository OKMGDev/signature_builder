...
const SignatureTable = ({ name, job, mobile, email, signatureType }) => {
  const companyLogo = COMPANY_LOGOS['Pure Leasing'];
  const company = COMPANY_DETAILS['Pure Leasing'];
  const isBrandSignature = signatureType === 'brand';
  const hasMobile = !isBrandSignature && mobile && mobile.trim() !== '';
  const hasEmail = !isBrandSignature && email && email.trim() !== '';
              ...
              <tbody>
                ...
                {hasEmail && (
                  <tr>
                    ...
                    <td valign="top" style={cell(SIGNATURE_STYLES.contact)}>
                      <a href={`mailto:${email.trim()}`} style={LINK_STYLE}>{email.trim()}</a>
                    </td>
                  </tr>
                )}
                <tr>
